...
let carousel = buildCarousel(items, {
  radiusX: radius,
  radiusY: radius,
  activeAngle: -90, // active is set at the top.
  draggable: true,
  onClick(element, self) {
    self.to(element, { duration: 1.5, ease: "power1.inOut" }, "short"); // how long it takes to transition on click
  },
...
document.querySelector("#prev").addEventListener("click", () => carousel.previous());

// Mobile Size
let mm = gsap.matchMedia();
mm.add("(max-width: 767px)", () => {
  radius = projectImages.offsetWidth / 2.8;
